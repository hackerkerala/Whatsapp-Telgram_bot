...
async function setPmMsg(pmMsg, method){
    try{
        ...
        method == 'update' ? 
        await setPm.db('WHATSAPPTELGRAM').collection('pmguard').updateOne({name: 'pmMsg'}, {$set: {pmMsg: pmMsg}}) : 
        await setPm.db('WHATSAPPTELGRAM').collection('pmguard').insertOne({name: 'pmMsg', pmMsg: pmMsg});
        return 'success'
    }catch(err){
        return 'failed'
    }finally{
...
async function readPmMsg(){
    try{
        ...
        var result = await read.db('WHATSAPPTELGRAM').collection('pmguard').find({name: 'pmMsg'}).toArray();
...
async function updateData(id, msgCount) {
    try {
        ...
        await update.db("WHATSAPPTELGRAM").collection("pmguard").updateOne({ number: id }, { $set: { msgCount: msgCount } })
...
async function readDb(id) {
    ...
    try {
        var read = await MongoClient.connect(DB_URL, { useNewUrlParser: true, useUnifiedTopology: true })
        var result = await read.db("WHATSAPPTELGRAM").collection("pmguard").find({ number: id }).toArray()
        if (result[0] != undefined) {
            return ({
                status: "found",
...
async function addToDb(id) {
    try {
        ...
        await addData.db("WHATSAPPTELGRAM").collection("pmguard").insertOne({ number: id, msgCount: 1, allowed: false })
        return "added"
    } catch (err) {
...
async function allow(id) {
    try {
        var update = await MongoClient.connect(DB_URL, { useNewUrlParser: true, useUnifiedTopology: true })
        await update.db("WHATSAPPTELGRAM").collection("pmguard").updateOne({ number: id }, { $set: { msgCount: 1, allowed: true } })
...
async function disAllow(id) {
    try {
        ...
        await updatewrite.db("WHATSAPPTELGRAM").collection("pmguard").updateOne({ number: id }, { $set: { msgCount: 1, allowed: false } })
        fs.readFile(path.join(__dirname, `../temp/${id}.json`), { encoding: 'utf8' },
            async function(err, data) {
                if (err) {} else {
